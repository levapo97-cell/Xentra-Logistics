...
const emptyLine = () => ({
    id: generateId('fpl'), producto_id: '', descripcion: '', cantidad: 1, costo_unitario: 0, impuesto_pct: 15,
});

export default function SupplierInvoiceForm() {
    const { id } = useParams();
    const navigate = useNavigate();
    const { supplierInvoices, addSupplierInvoice, updateSupplierInvoice, purchaseOrders, businessPartners, products, numberingRanges, getNextDocRef } = useApp();

    const isNew = !id || id === 'nueva';
    const existing = supplierInvoices.find(i => i.id === id);

    const [form, setForm] = useState(() => {
        if (!isNew && existing) return { ...existing, lineas: existing.lineas.map(l => ({ ...l })) };
        const range = numberingRanges.find(r => r.active && r.document === 'Factura Proveedor');
        const nextNum = range ? `${range.prefix}${String(range.current).padStart(6, '0')}` : 'FP-000001';
        const today = new Date().toISOString().split('T')[0];
        const due = new Date(Date.now() + 15 * 86400000).toISOString().split('T')[0];
        return { numero_documento: nextNum, num_factura_proveedor: '', orden_ref: '', fecha: today, fecha_vencimiento: due, proveedor_id: '', moneda: 'HNL', estado: 'Pendiente', observaciones: '', lineas: [emptyLine()] };
    });
    ...
    const handleSave = (newEstado) => {
        ...
        if (isNew) {
            const num = getNextDocRef('Factura Proveedor');
            if (num) data.numero_documento = num;
            const created = addSupplierInvoice(data);
            navigate(`/compras/facturas/${created.id}`);
        } else {
    ...
    return (
        <div className="space-y-5 max-w-6xl mx-auto pb-6">
            <div className="flex items-center gap-3">
                <Link to="/compras/facturas" className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100"><ArrowLeft size={18} /></Link>
                <div className="flex-1 min-w-0"><h2 className="page-title truncate">{isNew ? 'Nueva Factura Proveedor' : form.numero_documento}</h2></div>
            ...
            <div className="card">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div><label className="label">N° Interno</label><input className="input bg-gray-50 font-mono" value={form.numero_documento} readOnly /></div>
                    <div><label className="label">N° Factura Proveedor</label><input className="input font-mono" disabled={readOnly} value={form.num_factura_proveedor} onChange={e => setField('num_factura_proveedor', e.target.value)} placeholder="FAC-2026-00145" /></div>
                    <div>
                    ...
                    <div>
                        <label className="label">OC Ref. (Opcional)</label>
                        <select className="input" disabled={readOnly} value={form.orden_ref} onChange={e => setField('orden_ref', e.target.value)}>
                            <option value="">Sin referencia</option>
                            {purchaseOrders.map(o => <option key={o.id} value={o.numero_documento}>{o.numero_documento}</option>)}
                        </select>
                    </div>
                    <div>
                            ...
                            {form.lineas.map(line => {
                                ...
                                return (
                                    <tr key={line.id} className="group">
                                        ...
                                        <td className="py-2 pl-2 text-right font-semibold">{formatCurrency(t.total)}</td>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pb-4">
                <Link to="/compras/facturas" className="btn-secondary"><ArrowLeft size={16} /> Volver</Link>
                {!readOnly && (
                    <div className="flex flex-wrap gap-2">
                        <button className="btn-secondary" onClick={() => handleSave()}><Save size={16} /> Guardar</button>
                        {form.estado === 'Pendiente' && <button className="btn-success" onClick={() => handleSave('Pagada')}><CheckCircle size={16} /> Marcar Pagada</button>}
